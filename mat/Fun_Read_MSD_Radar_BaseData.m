...
while(1)
    ...
    BaseData.Radial(ii).Header.hne= ( fread(fid,1,'*int16')); 
    BaseData.Radial(ii).Header.vne= fread(fid,1,'*int16'); 
    BaseData.Radial(ii).Header.Reserved = fread(fid,14,'*uchar');    %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%注意：下面为了能方便的查阅每个仰角层面的数据，将数据进行重组，并将数据都转换成double类型
%1.先查找各个cut层面的起始和结束的位置
cut_number=1;
for ii=1:length(BaseData.Radial)
    if BaseData.Radial(ii).Header.RadialState==0 || BaseData.Radial(ii).Header.RadialState==3  %Cut Start || Volume Start
        cut_start_index(cut_number)=ii;
    end
    if((ii>10)&&(BaseData.Radial(ii).Header.RadialState==1)&&(BaseData.Radial(ii-1).Header.RadialState==2))
         cut_start_index(cut_number)=ii;
    end
    if BaseData.Radial(ii).Header.RadialState==2   %Cut END
        cut_end_index(cut_number)=ii;
        cut_number=cut_number+1;
    end
    if  BaseData.Radial(ii).Header.RadialState==4  %Volume END
        cut_end_index(cut_number)=ii;
    end

end

%2013.3.1 CJJ 发现对于RHI扫描模式，此时要将cut_number-1。
if (BaseData.Common.Task.ScanType ==2) 
    cut_number=cut_number-1;   
end

%2013.3.17 CJJ 发现对于PPI扇扫模式，此时要将cut_number-1。
if (BaseData.Common.Task.ScanType ==3) 
    cut_number=cut_number-1;   
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%注意：这里要确保两者相等。否则就说明数据的格式完全错误了！
%assert(BaseData.Common.Task.CutNumber==cut_number);  
% if (BaseData.Common.Task.CutNumber~=cut_number)
%     disp('错误：数据有严重的丢失，因此无法解出这个文件！');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if cut_number~=length(cut_start_index)
%    disp('由于最后一圈的数据已经丢失，因此将这个体扫的层数-1');
%    BaseData.Common.Task.CutNumber=BaseData.Common.Task.CutNumber-1;
% end

%2.再依次将各个径向上的数据填入各个cut
if(cut_number<BaseData.Common.Task.CutNumber)
     s = sprintf('错误：数据文件不完整！请选择完整的体扫数据文件！');
     msgbox(s);
     return;
end
for ii=1:BaseData.Common.Task.CutNumber
    ...
    utc_time=double( BaseData.Radial(cut_start_index(ii)).Header.Seconds + double(BaseData.Radial(cut_start_index(ii)).Header.microseconds)/1e6 );
    Data.Cut(ii).UTC=datestr(datenum(1970,1,1,0,0,0)+  utc_time/86400  ,'dd-mmm-yyyy HH:MM:SS' );

    Data.Cut(ii).MomentNumber=BaseData.Radial(cut_start_index(ii)).Header.MomentNumber;    %Moments available in this radial
    %if (Data.Cut(ii).MomentNumber==0)
    %    Data.Cut(ii).DataLength(1)= 0;
    %else
      Data.Cut(ii).DataLength=zeros(1,Data.Cut(ii).MomentNumber);
     %   Data.Cut(ii).DataLength= (  BaseData.Radial(cut_start_index(ii)).Header.Lengthofdata  / BaseData.Radial(cut_start_index(ii)).Header.MomentNumber  - 32 ) ...
     %       /  double(  BaseData.Radial(cut_start_index(ii)).Moment(1).Header.BinLength ) ; %计算得到径向上有多少个距离库  注意：这里就采用第一个Moment的BinLength
     for kk=1:Data.Cut(ii).MomentNumber
        binlen= (BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.BinLength );
        hdrlen=BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.Length;
         Data.Cut(ii).DataLength(kk)= hdrlen/int32(binlen);
     end;
   % end
    Data.Cut(ii).Resolution =BaseData.Common.Cut(ii).LogResolution;  %得到距离库的长度，单位：m
    Data.Cut(ii).RadialNumber=cut_end_index(ii)-cut_start_index(ii) +1;  %得到这个cut的 Radial Number，即相关脉冲组的总数
    ...
    for jj=1:Data.Cut(ii).RadialNumber
        addr=cut_start_index(ii)+jj-1 ;
        Data.Cut(ii).Azimuth(jj)=BaseData.Radial(addr).Header.Azimuth;
        Data.Cut(ii).Elevation(jj)=BaseData.Radial(addr).Header.Elevation;
        Data.Cut(ii).prf(jj)=BaseData.Radial(addr).Header.prf;
        Data.Cut(ii).hne(jj)=double(BaseData.Radial(addr).Header.hne)/-100.0;
        Data.Cut(ii).vne(jj)=double(BaseData.Radial(addr).Header.vne)/-100.0;
    end

    %2.3 得到每个cut下的基数据
    for kk=1:Data.Cut(ii).MomentNumber
        Data.Cut(ii).Moment(kk).Data=single(zeros(Data.Cut(ii).DataLength(kk) ,Data.Cut(ii).RadialNumber )); %预先开辟空间
        Data.Cut(ii).Moment(kk).DataType=   BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.DataType ;
        ...
        switch Data.Cut(ii).Moment(kk).DataType
            ...
            case 17
                Data.Cut(ii).Moment(kk).DataTypeString='SNRV';
            case 18
                Data.Cut(ii).Moment(kk).DataTypeString='SNRT';
            case 19
                Data.Cut(ii).Moment(kk).DataTypeString='PHASE'; %注意：这里比RVP9 输出的多一个“相位”的数据。用于研究大气折射率的变化
            case 21
                Data.Cut(ii).Moment(kk).DataTypeString='COP'; %相位变化。用于研究大气折射率的变化
            otherwise
                Data.Cut(ii).Moment(kk).DataTypeString='Spare';
        end

        Scale=  double(BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.Scale);
        Offset= double( BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.Offset);
        BinLength=    BaseData.Radial(cut_start_index(ii)).Moment(kk).Header.BinLength;
        ...
        for jj=1:Data.Cut(ii).RadialNumber
            ...
            if BinLength==2
                date_temp=single(BaseData.Radial(addr).Moment(kk).Data(2:2:end)) *256 + single( BaseData.Radial(addr).Moment(kk).Data(1:2:end));
            else
                date_temp=single(BaseData.Radial(addr).Moment(kk).Data);
            end
